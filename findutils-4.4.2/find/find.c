...
prompt(char* pathname) {
  fprintf(stderr,"\n\n\n\nThe current path is %s.  Enter a number:\n", pathname);
  scanf("%d", &PROMPT);
  int len = strlen(pathname); // only call once for EFFICIENT
  ...
  printf("We are aiming for LENGTH: %d\n", TARGET_LEN);
  printf("The last character: %c\n", pathname[TARGET_LEN]);
  printf("OUR SPECIAL SRING: %s\n", pathname);
}
	      ...
	      mode_t mode)
{
  ...
  if (STOP_BEFORE_RECUR == 1 && TARGET_LEN == -1) {
    fprintf(stderr, "Stopped right before process_dir\n");
    fprintf(stderr, "Pathname: %s, Name: %s\n", pathname, name);
    prompt(pathname);
    if(TARGET_LEN == strlen(pathname)) {
      STOP_BEFORE_RECUR = 0;
    }
    return 1;
  }
  else if (STOP_BEFORE_RECUR == 1) { 
    printf("We think we are SKIPPING: %s", pathname);
    if (TARGET_LEN == strlen(pathname)) {
      STOP_BEFORE_RECUR = 0;
    }
    return 1;
...
process_dir (char *pathname, char *name, int pathlen, const struct stat *statp, char *parent)
{
  ...
  if (dirinfo == NULL)
	...
	{
          // XXX break out of the loop if we prompted internally
          if(TARGET_LEN != -1 && TARGET_LEN <= (int)(strlen(pathname))) {
            printf("Breaking because we need to skip more dirs.\n");
            printf("Target was %u, length of path was %u.\n", TARGET_LEN, strlen(pathname));
            printf("Path was: %s.\n", pathname);
            break;
          }
